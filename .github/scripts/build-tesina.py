...
def main():
    ...
    # =====================================================================
    # 3.5. NUOVA VERSIONE: SCANSIONE AUTOMATICA DEI FILE BIBLIOGRAFICI
    # =====================================================================
    print("[*] Automatically scanning for bibliography files in docs/...")
    bib_dir = Path("docs")
    found_bib_files = {}

    # Cerca tutti i file .bib direttamente dentro la cartella 'docs'
    for file_path in bib_dir.glob("*.bib"):
        # Estrai il nome del file senza estensione (es. "bibliografia")
        stem = file_path.stem
        
        # Genera la chiave con le prime 3 lettere (es. "bib")
        key = stem[:3].lower() # .lower() per consistenza
        
        # Gestione di possibili conflitti di chiavi
        if key in found_bib_files:
            print(f"    [WARN] Key collision detected! The key '{key}' is generated by both '{found_bib_files[key]}' and '{file_path}'. The latter will be used.")
        
        # Aggiungi al dizionario
        path_value = str(file_path).replace('\\', '/')
        found_bib_files[key] = path_value
        print(f"    [OK] Found: {file_path} -> Mapped to key '{key}'")
    ...
    with open("README.md", "w", encoding='utf-8') as out:
        ...
        # =================================================================
        # 5. AGGIUNGI I DIV PER LE BIBLIOGRAFIE MULTIPLE (Funziona senza modifiche)
        # =================================================================
